...
{
    // v4-offline: report the display as DISCONNECTED so IOFramebuffer::updateOnline
    // computes nowOnline=false -> no IODisplay attach, no messageConnectionChange /
    // WindowServer/AGDC handoff (the GTrace kernel deadlock). The framebuffer nub still
    // exists (valid modes/pixel info) so the kernel walks past graphics init headless.
    switch (attribute) {
        case kConnectionEnable:
        case kConnectionCheckEnable:
            if (value) *value = 0;          // 0 = NOT enabled / offline
            return kIOReturnSuccess;
...
// v4-offline: no DDC/EDID -> no display published -> WindowServer never attaches.
bool HonorFB_v3::hasDDCConnect(IOIndex) { return false; }

IOReturn HonorFB_v3::getDDCBlock(IOIndex, UInt32, IOSelect,
                                 IOOptionBits, UInt8 *, IOByteCount *)
{
    return kIOReturnUnsupported;
}
